...
//   console.log(suggestions)
  ...
  const handleChange = async (value) => {
    ...
    try {
      const res = await axios.get(
        `${API_URL}/api/purchases/search?query=${value}`
      );
      setSuggestions(res.data.purchases);
      setShowList(true);
    } catch (err) {
  ...
  const handleSelect = async (supplier) => {
    ...
    const res = await axios.get(`${API_URL}/api/purchases/id/${supplier._id}`);
